...
#cblock[
  ...
  For any $A in RR^(m times n)$ with $"rank"(A) = n$, $A^+ = (A^T A)^(-1) A^T$ is a left inverse, i.e., $A^+ A = I$.
]

#cblock[
  == Full Row Rank (Def. 5.5.3)

  For any $A in RR^(m times n)$ with $"rank"(A) = m$, $A^+ = A^T (A A^T)^(-1)$ is a right inverse, i.e., $A A^+ = I$.
]

#cblock[
  == All Matrices (Def. 5.5.7)

  For any $R in RR^(m times n)$ with $A = C R$, $ A^+ = R^+ C^+. $ which can be rewritten as $ A^+ = R^T (C^T A R^T)^(-1) C^T. $
]
...
#cblock(fill: luma(240))[
  ...
  4. Prove that $A^+A$ is symmetric and that it is the projection matrix for the subspace $C(A^T)$. #line(length: 100%, stroke: 0.75pt) $A^+ A = (C R)^+ C R = R^+ C^+ C R = R^T (R R^T)^(-1) R$, following, $(A^+ A)^T = (R^T (R R^T)^(-1) R)^T = R^T ((R R^T)T)^(-1) R = R^T (R R^T)^(-1) R = A^+ A$.
]
