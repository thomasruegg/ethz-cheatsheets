...
#cblock[
  = Determinant (Def. 6.0.6) // TODO: I think it's now Def. 7.2.3
  ...
  #grid(
    ...
    gutter: 2pt,
    align: horizon,
    [$"det"(I) = 1$], [(Prop. 6.0.7)], // TODO: I think it's now Prop. 7.2.4
    [If $A in RR^(1 times 1)$, $"det"(A) = A$], [(Def. 7.2.3)],
    [$"det"(A^T) = "det"(A)$], [(Theo. 6.0.9)], // TODO: I think it's now Theo. 7.2.5
    [If $A$ is triangular, $"det"(A) = Pi_(k=1)^n A_(k k)$], [(Prop. 6.0.8)], // TODO: I think it's now Prop. 7.2.4
    [If $A$ is orthogonal, $"det"(A) = 1 "or" -1$], [(Prop. 6.0.10)], // TODO: I think it's now Prop. 7.2.4
    [$A$ is invertible if and only if $"det"(A) != 0$], [(Prop. 6.0.11)], //TODO: I think it's now Theo. 7.2.6
    [If $"det"(A) != 0$, $"det"(A^(-1)) = 1 / "det"(A)$], [(Prop 6.0.13)], //TODO: I think it's now Theo. 7.2.6
    [Given some $B in RR^(n times n)$, $"det"(A B) = "det"(A) dot "det"(B)$], [(Prop. 6.0.12)], // TODO: I think it's now Theo. 7.2.6
  )
]
...
- If we swap the rows of $A -> B$ once, then $det(B) = -det(A)$.
