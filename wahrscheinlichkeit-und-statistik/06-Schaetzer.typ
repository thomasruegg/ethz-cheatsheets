...
- Parameterraum $vartheta subset.eq RR^m$
- Familie von Wahrscheinlichkeitsmassen $(P_vartheta)_(vartheta in vartheta)$ auf $(Omega, F)$; für jedes Element im Parameterraum existiert ein Modell / Wahrscheinlichkeitsraum $(Omega, F, P_vartheta)$.
...
Ein Schätzer $T$ ist *erwartungstreu*, falls für alle $vartheta in vartheta$ gilt:
$ E_vartheta [T] = vartheta $

Sei $vartheta in vartheta$ und $T$ ein Schätzer. Der *Bias* (erwartete Schätzfehler) von $T$ im Modell $P_vartheta$ is definiert als:
$ E_vartheta [T] - vartheta $
...
Eine Folge von Schätzern $T^((n)), n in NN$, heisst *konsistent* für $vartheta$, falls $T^((n))$ für $n -> oo$ in $P_vartheta$-Wahrscheinlichkeit gegen $vartheta$ konvergiert, d.h. für jedes $vartheta in vartheta$ und jedes $epsilon > 0$ gilt:
...
Wenn $X_k$ unter $P_vartheta$ i.i.d. sind (analog mit $f_bold(x)$ und $f_X$):
$ p_bold(x)(x_1, ..., x_n; vartheta) = product_(k=1)^n p_X (x_k; vartheta) $
...
Daraus folgt:
$ E_vartheta [T_2] = (n-1)/n bb(V)_vartheta [X] != bb(V)_vartheta [X] $

Um einen erwartungstreuen Schätzer $T'$ für $(E_vartheta [X], bb(V)_vartheta [X])$ zu erhalten, verwendet man:
$ T_1' &= overline(X)_n \
  T_2' &= n/(n-1) T_2 = 1/(n-1) sum_(k=1)^n (X_k - overline(X)_n)^2 \
